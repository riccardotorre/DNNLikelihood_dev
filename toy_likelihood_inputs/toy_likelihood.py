...
##### Simplified likelihood for toy lilekihood and related quantities functions #####

def compute_SL_params(nbI_mu,nbI_cov,nbI_mu3):
    nbI_var = np.diag(nbI_cov)
    cI = -np.sign(nbI_mu3)*np.sqrt(2*nbI_var)*np.cos(4*np.pi/3+1/3*np.arctan(np.sqrt(8*np.power(nbI_var,3)/np.power(nbI_mu3,2)-1)))
    bI = np.sqrt(nbI_var-2*np.power(cI,2))
    aI = nbI_mu-cI
    rhoIJ = np.array([np.array([1/(4*cI[i]*cI[j])*(np.sqrt(np.power(bI[i]*bI[j],2)+8*cI[i]*cI[j]*nbI_cov[i,j])-bI[i]*bI[j]) for i in range(90)]) for j in range(90)])
    return aI, bI, cI, rhoIJ

def nbI_SL(pars,aI,bI,cI):
    theta = pars[1:]
    return aI+bI*theta+cI*theta*theta

def expected_SL(pars,aI,bI,cI):
    mu = pars[0]
    return np.array(nsI(mu)+nbI_SL(pars,aI,bI,cI))

def loglik_SL(pars,aI,bI,cI):
    obs = observed['all']
    exp = expected_SL(pars,aI,bI,cI)
    logfact = np.array(list(map(lambda x: np.math.lgamma(x+1), obs)))
    res = -1*logfact+obs*np.log(exp)-exp
    res = np.sum(res)
    if np.isnan(res):
...
def logprior_SL(pars,rhoIJ):
    mu = pars[0]
    theta = pars[1:]
    nuis_prior = -1/2*(np.dot(np.dot(theta,rhoIJ),theta)+len(theta)*np.log(2*np.pi))
    if (mu > -1) and (mu < 5):
        return nuis_prior-np.log(5)
    else:
        return nuis_prior-np.inf

def logprob_SL(pars,aI,bI,cI,rhoIJ):
    lprior=logprior_SL(pars,rhoIJ)
    if np.isinf(lprior):
        return lprior
    else:
        llik=loglik_SL(pars,aI,bI,cI)
        return llik+lprior
