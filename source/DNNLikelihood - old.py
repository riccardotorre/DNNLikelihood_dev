...
def save_results(folder, model, scalerX, scalerY, title, summary_text, X_train, X_test, Y_train, Y_test, tmuexact, tmuDNN, tmusample001, tmusample005, tmusample01, tmusample02, pars=[0], labels='None', plot_coverage=False, plot_distr=True, plot_corners=True, plot_tmu=False, batch_size=1, verbose=True):
    ...
    if plot_distr:
        ...
        plt.legend()#bbox_to_anchor=(0.4, 0.35))#, bbox_to_anchor=(0.3, 1), fontsize=15)#, bbox_transform=plt.gcf().transFigure)#, loc='upper right')
        plt.yscale('log')
        plt.xscale('log')
        #x1,x2,y1,y2 = plt.axis()
...
def generate_summary_text(summary_log,history,FREQUENTISTS_RESULTS):
    ...
    #print(summary_log['Loss'])
    #print(metric_name_unabbreviate(str(summary_log['Loss'])))
    #print("val_"+metric_name_unabbreviate(summary_log['Loss'])+'_best')
    #print(summary_log["val_"+metric_name_unabbreviate(summary_log['Loss'])+'_best'])
    #print('{0:1.2e}'.format(summary_log["val_"+metric_name_unabbreviate(summary_log['Loss'])+'_best']))
